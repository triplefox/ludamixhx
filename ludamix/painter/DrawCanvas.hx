...
class DrawCanvas {
	...
	public inline function slice(x : Int, y : Int, w : Int, h : Int) : DrawCanvas {
		if (w < 1 || h < 1) return null;
		var result = new DrawCanvas();
	...
	public inline function blit(src : DrawCanvas, x : Int, y : Int, ?w : Int=0, ?h : Int=0) : Void {
	...
	public inline function floodFill(x : Int, y : Int, v : Int) : DrawVector {
		var queue = new Array<Int>();
		var paints = new DrawVector();
		var seed = get(x, y);
	...
	public inline function floodMark(x : Int, y : Int) : {canvas:DrawCanvas, paint:DrawVector} {
		var queue = new Array<Int>();
		var paints = new DrawVector();
		var working = new DrawCanvas(); working.init(w, h, 0);
		var seed = get(x, y);
		queue.push(getIdx(x, y));
		while (queue.length > 0) {
			var node = queue.shift();
	...
	public inline function floodMark2(x : Int, y : Int) : {canvas:DrawCanvas, paint:DrawVector} {
		var queue = new Array<Int>();
		var paints = new DrawVector();
		var working = new DrawCanvas(); working.init(w, h, 0);
		var seed = get(x, y);
	 ...
	 * */
	public inline function dijkstraFlood(x : Int, y : Int, ?result : DrawCanvas) : {canvas:DrawCanvas,paint:DrawVector} {
		if (result == null) { result = new DrawCanvas(); result.init(w, h, -2); }
		var queue = new Array<Int>();
		var seed = get(x, y);
		var paint = new DrawVector();
		queue.push(getIdx(x, y));
		result.set(x, y, 0);
		while (queue.length > 0) {
			var node = queue.shift();
	...
	public inline function dijkstraPath4(x0 : Int, y0 : Int) : DrawVector {
		var x = x0;
		var y = y0;
	...
	public inline function dijkstraNaturalPath4(x0 : Int, y0 : Int, cx : Int, cy : Int) : DrawVector {
		var x = x0;
		var y = y0;
		var result = new DrawVector();
		var v = get(x0, y0);
	...
	public function getInwardSpiralSeed(seed : Int, not : Bool) {
		...
		var marking = new DrawCanvas(); marking.init(w, h, 0);
		var x = 0;
		var y = 0;
		var step = 0;
	...
	public function getIslands() {
		var result = new DrawCanvas(); result.init(w, h, 0);
		var isle = 1;
		var paints = new Array<DrawVector>();
	...
	public function setPaints(pr : DrawVector) {
	...
	public function setPaintsColor(pr : DrawVector, c : Int) {
		for (i0 in 0...pr.length) {
			var i = i0 * 3;
			var x = pr.data[i];
			var y = pr.data[i + 1];
